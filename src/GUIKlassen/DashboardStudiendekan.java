...
public class DashboardStudiendekan extends JFrame {
    ...
    public DashboardStudiendekan(DashboardStudiendekan dashboard) {
        ...
        // --- NEU: Benachrichtigungen für Studiendekan laden ---
        ladeBenachrichtigungenStudiendekan(pPortal);
    ...
    /**
     * Lädt alle Benachrichtigungen für den Studiendekan
     * und zeigt sie im Portal-Panel an.
     */
    private void ladeBenachrichtigungenStudiendekan(JPanel panel) {
        try (Connection conn = DBConnection.getConnection()) {
            PreparedStatement ps = conn.prepareStatement(
                "SELECT text, datum FROM benachrichtigungen WHERE mnr=0 ORDER BY datum DESC"
            );
            ResultSet rs = ps.executeQuery();

            int y = 70;
            while (rs.next()) {
                JLabel msg = new JLabel("• " + rs.getDate("datum") + ": " + rs.getString("text"));
